    ...
    const update = () => {
      ...
      for (let i = 0; i < count; i++) {
        for (let j = i + 1; j < count; j++) {
          ...
          if (dist < radius * 1.1) {
            const alpha = (1 - dist / (radius * 1.1)) * 0.08 * Math.min(items[i].alpha, items[j].alpha);
...
const ChatHibrido = () => {
  ...
  const handleSend = (text: string, isButton = false) => {
    ...
    setTimeout(() => {
      ...
      if (tQuery.includes('projet') || tQuery.includes('project') || tQuery.includes('🚀')) {
        reply = t.chat.replyProjects;
      } else if (tQuery.includes('orçament') || tQuery.includes('budget') || tQuery.includes('contrat') || tQuery.includes('hire') || tQuery.includes('💼')) {
        reply = t.chat.replyQuote;
      } else if (tQuery.includes('habilidad') || tQuery.includes('skill') || tQuery.includes('tecnolog') || tQuery.includes('stack') || tQuery.includes('🛠️')) {
        reply = t.chat.replySkills;
      } else if (tQuery.includes('whatsapp') || tQuery.includes('whats') || tQuery.includes('contat') || tQuery.includes('💬')) {
        reply = t.chat.replyQuote;
      } else {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto glass-panel border border-border rounded-3xl overflow-hidden flex flex-col shadow-2xl relative z-10">
      {/* Header */}
      <div className="bg-surface/50 backdrop-blur-md px-6 py-4 border-b border-border flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="relative w-10 h-10 rounded-full border border-primary/30 overflow-hidden bg-primary/10 flex items-center justify-center">
            <Image src="/eu.png" alt="Carlos André" fill className="object-cover" />
          </div>
          <div>
            <h4 className="font-black text-sm uppercase tracking-wider">{t.chat.assistantName}</h4>
        ...
        <span className="text-[9px] font-black bg-primary/10 text-primary border border-primary/20 px-3 py-1 rounded-full uppercase tracking-widest">{t.chat.assistantRole}</span>
      </div>

      {/* Message Area */}
      <div className="h-[340px] overflow-y-auto p-6 space-y-4 chat-scrollbar flex flex-col bg-bg/10">
        {messages.map((m) => (
        ...
        {isTyping && (
          <div className="flex justify-start">
            <div className="bg-surface border border-border text-txt rounded-2xl rounded-tl-none px-4 py-2.5 flex gap-1 items-center">
              <span className="w-1 h-1 bg-txt-muted rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
              <span className="w-1 h-1 bg-txt-muted rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
              <span className="w-1 h-1 bg-txt-muted rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
            </div>
          </div>
        )}
        <div ref={scrollRef} />
      ...
      <div className="p-3 bg-surface/20 border-t border-border flex flex-wrap gap-2 justify-center">
        <button onClick={() => handleSend(t.chat.btnProjects, true)} className="text-[9px] font-bold uppercase tracking-widest bg-bg/50 hover:bg-primary/20 border border-border hover:border-primary/45 px-3 py-2 rounded-full transition-all text-txt-muted hover:text-primary cursor-pointer">
          {t.chat.btnProjects}
        </button>
        <button onClick={() => handleSend(t.chat.btnQuote, true)} className="text-[9px] font-bold uppercase tracking-widest bg-bg/50 hover:bg-primary/20 border border-border hover:border-primary/45 px-3 py-2 rounded-full transition-all text-txt-muted hover:text-primary cursor-pointer">
          {t.chat.btnQuote}
        </button>
        <button onClick={() => handleSend(t.chat.btnSkills, true)} className="text-[9px] font-bold uppercase tracking-widest bg-bg/50 hover:bg-primary/20 border border-border hover:border-primary/45 px-3 py-2 rounded-full transition-all text-txt-muted hover:text-primary cursor-pointer">
          {t.chat.btnSkills}
        </button>
        <a href="https://api.whatsapp.com/send/?phone=21982665121&text=Olá+Carlos%2C+vi+seu+portfólio+e+gostaria+de+conversar+sobre+um+projeto.&type=phone_number&app_absent=0" target="_blank" rel="noopener noreferrer"
           onClick={() => handleSend(t.chat.btnWhatsApp, true)} className="text-[9px] font-bold uppercase tracking-widest bg-bg/50 hover:bg-green-500/20 border border-border hover:border-green-500/45 px-3 py-2 rounded-full transition-all text-txt-muted hover:text-green-500 cursor-pointer">
          {t.chat.btnWhatsApp}
        </a>
      </div>

      {/* Input */}
      <form onSubmit={(e) => { e.preventDefault(); handleSend(input); }} className="p-4 bg-surface/50 border-t border-border flex gap-2">
        <input
          type="text"
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder={t.chat.placeholder}
          className="flex-1 bg-bg/50 border border-border rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:border-primary/50 transition-colors"
        />
        <button type="submit" className="bg-primary hover:bg-accent text-white px-4 rounded-xl flex items-center justify-center transition-all cursor-pointer">
...
const HeroSection = () => {
  const { t } = useApp();
  return (
    <section id="home" className="min-h-screen flex flex-col justify-center items-center pt-28 pb-16 px-6 relative overflow-hidden">
            ...
            <div className="flex flex-wrap justify-center gap-6 md:gap-12">
              {[{ val: t.hero.stat1v, label: t.hero.stat1 }, { val: t.hero.stat2v, label: t.hero.stat2 }, { val: t.hero.stat3v, label: t.hero.stat3 }].map((s, i) => (
                <div key={i} className="flex flex-col items-center">
                  <span className="text-2xl md:text-3xl font-black text-primary">{s.val}</span>
                  <span className="text-[9px] font-bold uppercase tracking-widest text-txt-muted mt-0.5">{s.label}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="container mx-auto max-w-6xl">
          ...
          <FadeIn direction="right">
            <div className="relative group w-full max-w-[280px] aspect-square mx-auto md:mx-0">
              ...
              <div className="relative w-full h-full rounded-3xl overflow-hidden border border-border avatar-glow">
                <Image src="/eu.png" alt="Carlos André" fill className="object-cover transition-transform duration-700 group-hover:scale-105" />
              </div>
            </div>
          </FadeIn>
            ...
            <div className="text-center md:text-left">
              ...
              <div className="flex justify-center md:justify-start gap-3">
                <a href="https://github.com/techcarlosandre" target="_blank" rel="noopener noreferrer"
                  className="p-3.5 rounded-xl bg-surface/50 border border-border hover:bg-primary hover:text-white transition-all cursor-pointer"><GithubIcon size={18} /></a>
                <a href="https://www.linkedin.com/in/devcarlosandre/" target="_blank" rel="noopener noreferrer"
                  className="p-3.5 rounded-xl bg-surface/50 border border-border hover:bg-primary hover:text-white transition-all cursor-pointer"><LinkedinIcon size={18} /></a>
              </div>
            </div>
          </FadeIn>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <FadeIn delay={0.1}>
            <SpotlightCard>
              <div className="flex items-center gap-4 mb-5">
                <div className="p-3 bg-primary/20 rounded-xl text-primary"><MessageCircle size={20} /></div>
                <div>
                  <h4 className="font-black uppercase tracking-widest text-xs">{t.about.softSkillsTitle}</h4>
                  <p className="text-[9px] text-txt-muted font-bold uppercase tracking-widest">{t.about.softSkillsCount}</p>
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                {t.about.softSkills.map((s: string, i: number) => (
                  <span key={i} className="px-3 py-1.5 rounded-full border border-border text-txt-muted text-[9px] font-bold uppercase tracking-widest transition-colors hover:border-primary/30">{s}</span>
                ))}
              </div>
            </SpotlightCard>
          </FadeIn>
          ...
          <FadeIn delay={0.2}>
            <SpotlightCard>
              <div className="flex items-center gap-4 mb-5">
                <div className="p-3 bg-primary/20 rounded-xl text-primary"><Globe size={20} /></div>
                <div>
                  <h4 className="font-black uppercase tracking-widest text-xs">{t.about.langTitle}</h4>
                  <p className="text-[9px] text-txt-muted font-bold uppercase tracking-widest">{t.about.langCount}</p>
                </div>
              </div>
              <div className="space-y-3">
                {(t.about.langs as readonly { name: string; level: string }[]).map((l, i) => (
                  <div key={i} className="flex justify-between items-center">
                    <span className="font-bold text-xs md:text-sm">{l.name}</span>
                    <span className={`text-[8px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full border ${l.level === 'NATIVO' || l.level === 'NATIVE' ? 'text-primary border-primary/40 bg-primary/10' :
                ...
                >
                  <div className={`grid md:grid-cols-2 gap-0 ${!isEven ? 'md:[direction:rtl]' : ''}`}>
                    ...
                    <div className={`p-8 md:p-12 flex flex-col justify-center ${!isEven ? 'md:[direction:ltr]' : ''}`}>
                      ...
                      >
                        {/* Tech Tags & Actions (Visible only when expanded) */}
                        {'techs' in p && p.techs && (
                      ...
                      <div className="text-[10px] font-black text-primary uppercase tracking-widest mt-2 flex items-center gap-1">
                        {isExpanded ? '▲ Recolher detalhes' : '▼ Clique para expandir detalhes'}
                      </div>
                    </div>
                  </div>
                </motion.div>
